...
export const Auth = {
  private: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res.status(403).json({ error: 'Não autorizado. Token ausente ou em formato inválido.' });
    } catch (error) {
      if (error instanceof JWT.JsonWebTokenError) {
        console.error("Erro na verificação do token:", {status: 404});
        res.status(403).json({ error: 'Não autorizado. Token inválido ou expirado.' });
      } else {
    ...
    if (userId) {
      try {
        ...
        if (user && user.role === 'user') {
          ...
          res.status(403).json({ error: 'Acesso proibido. Você não tem permissão para acessar esta rota como usuário.' });
        }
      } catch (error) {
      ...
      res.status(403).json({ error: 'Acesso proibido. Token inválido ou usuário não encontrado.' });
    }
  },
    ...
    if (userId) {
      try {
        ...
        if (user && user.role === 'admin') {
          ...
          res.status(403).json({ error: 'Acesso proibido. Você não tem permissão de administrador para acessar esta rota.' });
        }
      } catch (error) {
...
function getUserIdFromToken(authorizationHeader: string | undefined): number | null {
